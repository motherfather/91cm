<template>
  <div>
    <li class="list-unstyled chat-message" v-if="!isMsgByLoginUser">
      <div
        class="myflex"
        @mouseover="showMsgOption(msg.id)"
        @mouseleave="hideMsgOption(msg.id)"
      >
        <div class="icon">
          <slot name="m-icon">
            <img
          ...
          <div style="display:flex;">
            <slot name="m-content">
              ...
              <div style="display:flex;align-items: flex-end;">
                    ...
                    <b-col v-for="(file, index) in msg.files" :key="index">
                      <a @click="fileDownload(file)">
                        ...
                        <p style="margin:0px;">
                          file size :
                          {{
                            (file.file_size / 1024).toLocaleString(undefined, {
                ...
                <span style="font-size: 11px; margin:0px 3px; width:53px; ">{{
                  msg.str_send_date
                }}</span>
                <a
                  class="verti-align confirmMsgDel"
                  :id="'confirmMsgDel' + msg.id"
        ...
        <div style="display:flex;">
          <slot name="m-content">
            ...
            <div
              v-if="checkMsgType"
              v-html="textbyFilter(msg.content)"
              class="my-message mychat-content"
            ></div>
...
export default {
  ...
  methods: {
    ...
    showMsgOption: function(msgId) {
      if (
        this.msg.delete_yn === "N" &&
        (this.isMsgByLoginUser || this.isAdmin())
      ) {
        this.hideMsgOption();
    ...
    textbyFilter: function(content) {
      ...
      const urlRegexp = new RegExp(
        /(http(s)?:\/\/|www.)([a-z0-9\w]+\.*)+[a-z0-9]{2,4}([\/a-z0-9-%#?&=\w])+(\.[a-z0-9]{2,4}(\?[\/a-z0-9-%#?&=\w]+)*)*/
      );
      let result = "";
      ...
      return this.$options.filters.highlight(
        content,
    ...
    fileDownload: function(file) {
      this.$http
        .get("/api/file/download/" + file.server_name, {
          responseType: "blob"
        })
        .then(res => {
