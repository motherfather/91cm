      ...
      <template v-if="connectionCheck">
        ...
        <div class="page-wrap">
          ...
          <div
            ...
            :class="{
              'disactive-padding': $store.state.selectComponent == 'main'
            }"
          >
            <div :class="{ row: isVideoMode, 'no-gutters': isVideoMode }">
              <div :class="{ col: isVideoMode, 'col-3': isVideoMode }">
                <NoChannel
                  v-if="
                    $store.state.channelList[0] == null &&
                      $store.state.selectComponent == 'main'
                  "
                />
                <keep-alive v-else>
          ...
          <footer class="footer">
            <div class="w-100 clearfix">
              <span class="text-center text-sm-left overline d-md-inline-block"
                >Copyright © 2018 ThemeKit</span
              >
            </div>
          </footer>
        </div>
...
export default {
  ...
  computed: {
    whichComponent() {
      AboutChannel.updateLastAccessStatus(
        this.$store.state.oldComponent,
        this.$store.state.selectComponent
      );
